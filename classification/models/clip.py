...
class CLIP(nn.Module):
    def __init__(
        ...
        is_lora_image,
        is_lora_text,
        download_root="model_clip",
        clip_version="ViT-B/16",
        is_image_full_finetuning=False,
    ):
        super().__init__()
        ...
        self.is_image_full_finetuning = is_image_full_finetuning 

        # TODO: change the number of templates
        self.templates = TEMPLATES_SMALL[:1]

        self.clip, _ = clip.load(clip_version, device="cpu", download_root=download_root)

        # visual model
        if is_lora_image:
            
            if self.clip_version != "RN50":
                if not self.is_image_full_finetuning:
...
#     @staticmethod
    ...
    def set_learnable_params(self):
        ...
        if self.is_lora_image:
            if self.clip_version != "RN50" and not self.is_image_full_finetuning:
