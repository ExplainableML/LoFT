...
# ImageNet
def labels_from_names(data_dir, subset_names):
    fpath = ospj(data_dir, "imagenet_class_index_full_classname.json")
    with open(fpath, "r") as f:
        dct = json.load(f)
        label_to_name = {k: v[1] for k, v in dct.items()}

    # check if wrong name exist
    nonexist_list = []
    for query_name in subset_names:
        exist = False
        for label, names in label_to_name.items():
            names = [_n.strip() for _n in names.split(",")]
            if query_name in names:
                exist = True
                break
        if not exist:
            nonexist_list.append(query_name)

    if len(nonexist_list) != 0:
        raise ValueError(f"names for {nonexist_list} should be changed.")
    ...
    for query_name in subset_names:
        for label, names in label_to_name.items():
            names = [_n.strip() for _n in names.split(",")]
            if query_name in names:
                subset_labels.append(int(label))
                break
...
# def set_data_root(dataset_name, data_root):
...
#     data_root = mch(train=train, test=test)
#     return data_root
...
class GetBoundingBoxes(object):
    def __init__(self, metadata, resize_length):
        self.image_ids = get_image_ids(metadata)
        self.original_bboxes = self.get_bounding_boxes(metadata)
        self.image_sizes = self.get_image_sizes(metadata)
        self.gt_bboxes = self.load_resized_boxes(resize_length)

    def get_bounding_boxes(self, metadata):
        """
    ...
    def get_image_sizes(self, metadata):
        """
    ...
    def load_resized_boxes(self, resize_length):
        resized_bbox = {image_id: [
            self.resize_bbox(bbox, self.image_sizes[image_id],
                             (resize_length, resize_length))
            for bbox in self.original_bboxes[image_id]]
            for image_id in self.image_ids}
        return resized_bbox

    def resize_bbox(self, box, image_size, resize_size):
        """
        Args:
...
# The code is from https://github.com/naver/trex/blob/master/multi_crop.py
##################################################
...
def get_random_sample(i, j, h, w, height, width):
    scale_sample = (h * w) / (height * width)
    ratio_sample = w / h
    i_sample = i / h
    j_sample = j / h
    return scale_sample, ratio_sample, i, j

def random_augment_vars_sample():
    # for RandomHorizontalFlip
    is_flip = 1. if torch.rand(1) < 0.5 else 0.
    ...
    for _ in range(10):
        area_scale = torch.empty(1).uniform_(scale[0], scale[1]).item()
        ...
        if 0 < w <= width and 0 < h <= height:
            ...
            sample = get_random_sample(i, j, h, w, height, width)
            return *sample, is_flip
    ...
    i = (height - h) // 2
    j = (width - w) // 2
    sample = self.get_random_sample(i, j, h, w, height, width)
    return *sample, is_flip
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def get_random_sample(self, i, j, h, w, height, width):
        scale_sample = (h * w) / (height * width)
        ratio_sample = w / h
        i_sample = i / h
        j_sample = j / h
        return scale_sample, ratio_sample, i, j
        # return scale_sample, ratio_sample, i_sample, j_sample
    ...
    def forward(self, img, get_params=False, sample=None):
        """
        Args:
        ...
        """
        if sample is None:
            i, j, h, w, sample = self.get_params(img, self.scale, self.ratio)
        else:
            _, height, width = F.get_dimensions(img)
            scale_sample, ratio_sample, i, j = sample
            h = int(round(math.sqrt(scale_sample * height * width / ratio_sample)))
            w = int(round(math.sqrt(scale_sample * height * width * ratio_sample)))
        img_return = F.resized_crop(img, i, j, h, w, self.size, self.interpolation, antialias=self.antialias)
        if get_params:
            return img_return, sample
        else:
            return img_return
...
class RandomHorizontalFlip(torch.nn.Module):
    ...
    def forward(self, img, get_params=False):
        if torch.rand(1) < self.p:
            img_return = F.hflip(img)
            is_flip = 1.
        else:
            img_return = img
            is_flip = 0.
        if get_params:
            return img_return, is_flip
        else:
            return img_return
